...
Вот что делает клиент (все операции проводятся клиентским JavaScript прямо в браузере):
  ...
  Давайте отдельно выделим преимущества и недостатки вышеуказанного метода по сравнению с традиционной рассылкой скриптом:

  - Плюсы:
    - транзакция, требующая токены, стоит мало газа, причем это число - <b>константа</b>, зависящая от размера "белого" списка
    - после запуcка контракта он не требует ни малейшей поддержки, вся активность обеспечивается пользователями
